...
def _build_week_plans(
    *,
    ...
    panel_key: str,
    mon: date,
    familias_panel: pd.DataFrame,
    family_lookup: dict[str, dict],
    ubs_lat: float,
    ubs_lon: float,
    n_dia: int,
) -> list[dict]:
    """Para cada dia Seg–Sex, devolve {dia, ordem, status, source} usando snapshot se existir
    ou regenerando via selecao_dia. Snapshots/sugestões de dias anteriores informam o rerank dos seguintes."""
    plans: list[dict] = []
    for i in range(5):
        d = mon + timedelta(days=i)
        snap = load_snapshot(equipe_id, int(cluster_id), panel_key, d)
        if snap is not None:
            ordem_d = list(snap.get("ordem", []))
            status_d: dict[str, str] = {}
            for fid in ordem_d:
                member_status_map = snap.get("member_status", {}).get(fid, {})
                if member_status_map:
                    status_d[fid] = derive_family_status(member_status_map)
                else:
                    status_d[fid] = normalize_status(snap.get("status", {}).get(fid))
            plans.append({"dia": d, "ordem": ordem_d, "status": status_d, "source": "snapshot"})
            continue

        # Sem snapshot — gerar sugestão. Usa snapshots/plans gerados em dias anteriores
        # para alimentar prev_status e o rerank mensal.
        prior_snaps = snapshots_in_range(
            equipe_id, int(cluster_id), panel_key, month_start(d), d - timedelta(days=1)
        )
        for prior in plans:
            if prior["source"] == "suggested" and prior["dia"] >= month_start(d):
                prior_snaps.append({
                    "dia": prior["dia"].isoformat(),
                    "ordem": prior["ordem"],
                    "status": prior["status"],
                    "member_status": {},
                })
        prior_snaps.sort(key=lambda s: s.get("dia", ""))

        prev_status = None
        if prior_snaps:
            last = prior_snaps[-1]
            prev_status = {p: normalize_status(s) for p, s in last.get("status", {}).items()}

        monthly_outcomes = family_outcomes_in_range(prior_snaps, family_lookup)
        ...
        forced_front_pids = {
            ...
            and date.fromisoformat(outcome["date"]) <= (d - timedelta(days=1))
        }
        ordem_d = selecao_dia(
            familias_panel, ubs_lat, ubs_lon, int(n_dia),
            prev_status, exclude_pids=exclude_pids, forced_front_pids=forced_front_pids,
        )
        status_d = {fid: STATUS_PENDENTE for fid in ordem_d}
        plans.append({"dia": d, "ordem": ordem_d, "status": status_d, "source": "suggested"})
    return plans


def _weekly_grid(plans: list[dict], family_lookup: dict[str, dict]) -> pd.DataFrame:
    family_ids: list[str] = []
    seen: set[str] = set()
    for plan in plans:
        for fid in plan["ordem"]:
            if fid not in seen:
                seen.add(fid)
    ...
    for fid in family_ids:
        fam = family_lookup.get(fid, {})
        row = {
            "Família": f"{fid[:14]}…",
            "Prioridade": fam.get("priority_band", "—"),
            "Pessoas": fam.get("member_count", "—"),
        }
        for i, plan in enumerate(plans):
            d = plan["dia"]
            col_label = f"{WEEKDAY_LABELS[i]} {d.strftime('%d/%m')}"
            if fid not in plan["ordem"]:
                row[col_label] = ""
                continue
            if plan["source"] == "suggested":
                row[col_label] = WEEKLY_SUGGESTED_TAG
            else:
                s = plan["status"].get(fid, STATUS_PENDENTE)
                row[col_label] = f"{WEEKLY_STATUS_EMOJI.get(s, '')} {s}"
        rows.append(row)
...
def _weekly_kpis(plans: list[dict], family_lookup: dict[str, dict]) -> dict:
    visitas_planejadas = 0
    sugeridas = 0
    family_final: dict[str, str] = {}
    family_dates: dict[str, str] = {}
    for plan in plans:
        visitas_planejadas += len(plan["ordem"])
        if plan["source"] == "suggested":
            sugeridas += len(plan["ordem"])
            continue
        snap_date = plan["dia"].isoformat()
        for fid in plan["ordem"]:
            derived = plan["status"].get(fid, STATUS_PENDENTE)
            prev_date = family_dates.get(fid, "")
            if snap_date >= prev_date:
                family_final[fid] = derived
                family_dates[fid] = snap_date

    atendidos = sum(1 for s in family_final.values() if s == STATUS_ATENDIDO)
    nao_atendeu = sum(1 for s in family_final.values() if s == STATUS_NO_SHOW)
    recusou = sum(1 for s in family_final.values() if s == STATUS_RECUSOU)
    prioritarios_pendentes = sum(
        1
        for fid, fam in family_lookup.items()
        if fam.get("priority_band") == FAMILY_PRIORITY_HIGH
        and family_final.get(fid) != STATUS_ATENDIDO
    )
    familias_unicas = len({fid for plan in plans for fid in plan["ordem"]})
    return {
        ...
        "recusou": recusou,
        "prioritarios_pendentes": prioritarios_pendentes,
...
def render_weekly_view(
    *,
    ...
    mon = week_start(selected_date)
    fri = mon + timedelta(days=4)
    plans = _build_week_plans(
        equipe_id=equipe_id, cluster_id=cluster_id, panel_key=panel_key, mon=mon,
        familias_panel=familias_panel, family_lookup=family_lookup,
        ubs_lat=ubs_lat, ubs_lon=ubs_lon, n_dia=n_dia,
    )

    ctx_cols = st.columns([3, 2, 2, 3])
    ctx_cols[0].markdown(
        f"**Semana de {mon.strftime('%d/%m')} a {fri.strftime('%d/%m')}**  \n"
        f"<span style='color:#6c757d;font-size:12px'>Seg–Sex · sugestão da semana</span>",
    ...
    ctx_cols[2].markdown(
        f"**{acs_label}**  \n<span style='color:#6c757d;font-size:12px'>{panel_size} domicílios na micro-área</span>",
        unsafe_allow_html=True,
    )

    kpis = _weekly_kpis(plans, family_lookup)
    cobertura_pct = (kpis["familias_unicas"] / panel_size * 100) if panel_size else 0
    ctx_cols[3].markdown(
        f"**Cobertura prevista**  \n<span style='color:#6c757d;font-size:12px'>"
        f"{kpis['familias_unicas']}/{panel_size} domicílios ({cobertura_pct:.0f}%)</span>",
        unsafe_allow_html=True,
    )
    if panel_size:
        st.progress(min(kpis["familias_unicas"] / panel_size, 1.0))

    m1, m2, m3, m4, m5 = st.columns(5)
    m1.metric("Visitas planejadas", kpis["visitas_planejadas"], help="Soma das listas dos 5 dias úteis (snapshots + sugestões).")
    m2.metric("Sugeridas", kpis["sugeridas"], help="Dias sem snapshot — sugestão gerada no momento.")
    m3.metric("Atendidos", kpis["atendidos"])
    m4.metric("Não atendeu", kpis["nao_atendeu"])
    m5.metric("Prioritários pendentes", kpis["prioritarios_pendentes"])

    st.subheader("Domicílios da semana")
    st.caption(
        "✅ atendeu · 🟡 parcial · ⚠️ não atendeu · 🚫 recusou · ⏳ pendente · 📝 sugerido (sem snapshot) · célula vazia = fora da lista do dia"
    )

    grid = _weekly_grid(plans, family_lookup)
    if grid.empty:
        st.info("Nenhum domicílio elegível na semana — o painel pode estar cumprido no mês.")
        return
    ...
    st.dataframe(
        ...
        hide_index=True,
        column_config={
            "Família": st.column_config.TextColumn(width="medium"),
            "Prioridade": st.column_config.TextColumn(width="small"),
            "Pessoas": st.column_config.NumberColumn(width="small"),
        },
    )
...
with st.sidebar:
    ...
    view_mode = st.radio(
        ...
        horizontal=True,
        key="VIEW_MODE",
        help="Dia: planejamento e marcação de status. Semana: resumo Seg–Sex somente leitura.",
    )
    st.divider()
...
if view_mode == "Semana":
    render_weekly_view(
        ...
        acs_label=acs_label_by_cluster[cluster_id],
        family_lookup=family_lookup,
        panel_size=int(len(familias_panel)),
        familias_panel=familias_panel,
